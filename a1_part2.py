...
def get_sign_group(sign):
    ...
    3
    '''
    signs=SIGN_GROUPS.replace("[","").replace("]","").split("-")
...
def find_astrological_sign(month, date):
    ...
    signs=SIGNS.split(";")
    for i in range(1,13):
        if (int(signs[i][:2])==month and  (int(signs[i][3:5])<=date) or int(signs[i][6:8])==month and int(signs[i][9:11])>=date):
            return signdate[12:]
...
if __name__ == "__main__":
    ...
    print("You are " + str(calculate_love_score(name1.lower(), name2.lower(), bday1, bday2)) +"% compatible in love!")# NOTE FROM BOB: DO NOT EDIT THIS STRING
# SIGNS_GROUPS is a string representing a group for each sign:
# - Group 0 has Aries, Leo, and Saggitarius
# - Group 1 has Taurus, Virgo and Capricorn
# - Group 2 has Gemini, Libra, Aquarius
